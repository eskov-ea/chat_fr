...
class MessagesRepository  {
  ...
  Future <Map<String, dynamic>?> getNewUpdatesOnResume(time) => messagesProvider.getNewUpdatesOnResume(time);
  ...
  Future <String> sendMessage({required int dialogId, required String? messageText, required String? uuid, required int? parentMessageId, required String? filetype, required Uint8List? bytes, required String? filename, required String? content}) =>
      _messageHelper.sendMessage(bytes: bytes, messageText: messageText, uuid: uuid, parentMessageId: parentMessageId, dialogId: dialogId, filetype: filetype, filename: filename, content: content);
